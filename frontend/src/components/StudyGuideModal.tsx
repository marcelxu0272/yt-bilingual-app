...
export const StudyGuideModal: React.FC<StudyGuideModalProps> = ({ isOpen, guide, transcript, favorites, onClose, onSeek, onToggleExpressionFavorite }) => {
    ...
    const activeQuestionIndex = guide?.questions.length ? Math.min(questionIndex, guide.questions.length - 1) : 0;
    ...
    const nextQuestion = () => {
        setSelectedAnswer(null);
        setQuestionIndex(Math.min(activeQuestionIndex + 1, (guide?.questions.length ?? 1) - 1));
    };
                ...
                <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/45 backdrop-blur-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    <motion.div initial={{ y: 18, scale: 0.98, opacity: 0 }} animate={{ y: 0, scale: 1, opacity: 1 }} exit={{ y: 10, scale: 0.99, opacity: 0 }} transition={{ type: 'spring', stiffness: 380, damping: 32 }} className="w-full max-w-3xl max-h-[88vh] flex flex-col glass-panel rounded-2xl overflow-hidden">
                        <div className="flex items-center justify-between px-6 py-5 border-b border-white/5 shrink-0">
                            ...
                            <button onClick={close} className="p-2 rounded-full text-zinc-500 hover:text-zinc-200 hover:bg-white/5 transition-colors"><X className="w-5 h-5" /></button>
                        </div>
                        ...
                        <div className="overflow-y-auto custom-scrollbar p-6 min-h-0">
                            ...
                            {tab === '测验' && <div>{guide.questions.length === 0 ? <p className="text-sm text-zinc-500">暂无测验题。</p> : (() => { const question = guide.questions[activeQuestionIndex]; return <div><div className="flex items-center justify-between mb-4"><span className="text-[11px] text-zinc-500 tabular-nums">第 {activeQuestionIndex + 1} / {guide.questions.length} 题</span>{selectedAnswer !== null && <span className={`inline-flex items-center gap-1 text-xs ${selectedAnswer === question.answer ? 'text-emerald-500' : 'text-amber-500'}`}><Check className="w-3.5 h-3.5" />{selectedAnswer === question.answer ? '回答正确' : '查看正确答案'}</span>}</div><p className="text-lg leading-relaxed text-zinc-100 mb-5">{question.question}</p><div className="space-y-2">{question.options.map((option, index) => { const isCorrect = selectedAnswer !== null && index === question.answer; const isWrongSelection = selectedAnswer === index && index !== question.answer; return <button key={`${index}-${option}`} disabled={selectedAnswer !== null} onClick={() => setSelectedAnswer(index)} className={`w-full text-left p-3 rounded-xl border text-sm transition-colors ${isCorrect ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-500' : isWrongSelection ? 'bg-amber-500/10 border-amber-500/30 text-amber-500' : 'bg-white/5 border-white/5 text-zinc-300 hover:bg-white/10'}`}>{String.fromCharCode(65 + index)}. {option}</button>; })}</div>{selectedAnswer !== null && <div className="mt-5 p-4 rounded-xl bg-blue-500/10 border border-blue-500/20 text-sm text-zinc-300"><p>{question.explanation}</p>{activeQuestionIndex < guide.questions.length - 1 && <button onClick={nextQuestion} className="mt-3 text-blue-500 hover:text-blue-400">下一题</button>}</div>}</div>; })()}</div>}
                        </div>
                    </motion.div>
                </motion.div>
            )}
